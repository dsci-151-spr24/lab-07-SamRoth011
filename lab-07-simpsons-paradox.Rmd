...
1.  What type of study do you think these data come from: observational or experiment?
    ...
    ***This study is observational, as there was no assigned treatment group for people.***

2.  How many observations are in this dataset?
    What does each observation represent?

***There are `r nrow(Whickham)` observations in the dataset. Each observation represents an individual person.***
...
***There are `r ncol(Whickham)` variables in the dataset. Outcome is categorical, smoker is categorical, and age is numerical.***

```{r}
ggplot(Whickham, aes(y = outcome, fill = outcome)) +
  geom_bar() +
  ggtitle("Distribution of Outcome")

```
```{r}
ggplot(Whickham, aes(y = smoker, fill = smoker)) +
  geom_bar() +
...
```{r}
ggplot(Whickham, aes(y = outcome, fill = smoker)) +
  geom_bar(position = "fill") +
  ggtitle("Relationship of Smoking and Health")
...
***This does not meet what I expected. It appears that those who smoke have a higher chance of living 20 years later.***
...
ggplot(Whickham, aes(y = outcome, fill = smoker)) +
  geom_bar(position = "fill") +
  ggtitle("Distribution of Outcome by Age") +
  facet_wrap(~age_cat)

```
***In the plot it appears that smokers have a higher probability of being dead of each age category.***
